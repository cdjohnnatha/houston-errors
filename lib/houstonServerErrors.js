...
/**
 * @param {Object} optionals - optionals Object
 * @param {string} optionals.message - Custom Message (Optional).
 * @param {string|Object} optionals.data - Custom Message (Optional).
 */
function InternalServerError({ message, data } = {} ) {
  return DefaultError(INTERNAL_SERVER_ERROR, { message: message, data: data });
}
...
function NotImplemented({ message, data } = {} ) {
  return DefaultError(NOT_IMPLEMENTED, { message: message, data: data });
}
...
function BadGateway({ message, data } = {} ) {
  return DefaultError(BAD_GATEWAY, { message: message, data: data });
}
...
function ServiceUnavailable({ message, data } = {} ) {
  return DefaultError(SERVICE_UNAVAILABLE, { message: message, data: data });
}
...
function GatewayTimeout({ message, data } = {} ) {
  return DefaultError(GATEWAY_TIMEOUT, { message: message, data: data });
}
...
function HTTPVersionNotSupported({ message, data } = {} ) {
  return DefaultError(HTTP_VERSION_NOT_SUPPORTED, { message: message, data: data });
}
...
function VariantAlsoNegotiates({ message, data } = {} ) {
  return DefaultError(VARIANT_ALSO_NEGOTIATES, { message: message, data: data });
}
...
function InsufficientStorage({ message, data } = {} ) {
  return DefaultError(INSUFFICIENT_STORAGE, { message: message, data: data });
}
...
function LoopDetected({ message, data } = {} ) {
  return DefaultError(LOOP_DETECTED, { message: message, data: data });
}
...
function BandwidthLimitExceeded({ message, data } = {} ) {
  return DefaultError(BANDWIDTH_LIMIT_EXCEEDED, { message: message, data: data });
}
...
function NotExtended({ message, data } = {} ) {
  return DefaultError(NOT_EXTENDEED, { message: message, data: data });
}
...
function NetworkAuthenticationRequired({ message, data } = {} ) {
  return DefaultError(NETWORK_AUTHENTICATION_REQUIRED, { message: message, data: data });
}
