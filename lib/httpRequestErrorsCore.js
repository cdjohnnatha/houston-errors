...
function Logger(error) {
  ...
  async function CleaningData(data) {
    ...
    await Object.entries(cleanedData).forEach((value, key) => {
      if(value[key] === undefined || value[key] === null) {
        logger.warn(`The key ${value[key]} is being ignored because have an undefined, empty or null value`);
  ...
  */
  async function DefaultError(errorObject, message = undefined, data = undefined) {
   ...
   * @param {string} data - Aditional data (Optional).
  */
 function CustomError(code, error, name, message, data) {
